...
public class Gestor {
    ...
    public void guardar(Object o) throws Exception {// recibe un objeto, lo puedo utilar para guardar cualquier objeto cliente especialidad etc

        boolean guardar = false;
        Transaction tx = null; // crea una transaccion, me asegura que sino se cumple de forma correcta todas las sentencias que componen 
        //la transaccion se dehace la totalidad de acciones que se hayan realizado en la base de datos, se vuelve al estado antes de haber ejecutado la transaccion
        try {
            if (!sesion.getTransaction().isActive()) {
                tx = sesion.beginTransaction();
                guardar = true;
            }
            sesion.saveOrUpdate(o); //llama a guardar
            if (guardar) {
                tx.commit(); //
                System.out.println("objeto persistido.");
            }
        } catch (Exception e) {
            if (guardar) {//si algo falla en la transaccion se realiza de forma automatica el rollback para volver hacia atras
                tx.rollback();
            }
